...
int main(int argc, char *argv[])
{
	...
	fctx.fs = main_fs;
	main_fs->priv_data = &fctx;

	ret = 3;

	if (ext2fs_has_feature_journal_needs_recovery(main_fs->super)) {
		if (fctx.norecovery) {
			...
			err = ext2fs_run_ext3_journal(&main_fs);
			if (err) {
				printf(_("%s: %s.\n"), fctx.device,
				       error_message(err));
				printf(_("Please run e2fsck -fy %s.\n"),
			...
			ext2fs_clear_feature_journal_needs_recovery(main_fs->super);
			ext2fs_mark_super_dirty(main_fs);
		} else {
	...
	if (!fctx.ro) {
		if (ext2fs_has_feature_journal(main_fs->super))
			printf(_("%s: Writing to the journal is not supported.\n"),
			       fctx.device);
		err = ext2fs_read_inode_bitmap(main_fs);
		if (err) {
			translate_error(main_fs, 0, err);
			goto out;
		}
		err = ext2fs_read_block_bitmap(main_fs);
		if (err) {
			translate_error(main_fs, 0, err);
			goto out;
		}
	...
	if (!(main_fs->super->s_state & EXT2_VALID_FS))
		printf("%s", _("Warning: Mounting unchecked fs, running e2fsck "
		       "is recommended.\n"));
	if (main_fs->super->s_max_mnt_count > 0 &&
	    main_fs->super->s_mnt_count >= main_fs->super->s_max_mnt_count)
		printf("%s", _("Warning: Maximal mount count reached, running "
		       "e2fsck is recommended.\n"));
	if (main_fs->super->s_checkinterval > 0 &&
	    (time_t) (main_fs->super->s_lastcheck +
		      main_fs->super->s_checkinterval) <= time(0))
		printf("%s", _("Warning: Check time reached; running e2fsck "
		       "is recommended.\n"));
	if (main_fs->super->s_last_orphan)
		printf("%s",
		       _("Orphans detected; running e2fsck is recommended.\n"));

	if (main_fs->super->s_state & EXT2_ERROR_FS) {
		printf("%s",
...
out:
	if (main_fs) {
		err = ext2fs_close(main_fs);
		if (err)
			com_err(argv[0], err, "while closing fs");
		if (main_fs->device_name)
			free(main_fs->device_name);
		main_fs = NULL;
	}
	return ret;
}
			     ...
			     const char *file, int line)
{
	struct timespec now;
	int ret = err;
	struct fuse2fs *ff = fs ? fs->priv_data : NULL;
	FILE *err_fp = ff ? ff->err_fp : stderr;
	int is_err = 0;
	char *device_name = (fs && fs->device_name) ? fs->device_name : "???";
...
no_translation:
	...
	if (ino)
		fprintf(err_fp, "FUSE2FS (%s): %s (inode #%d) at %s:%d.\n",
			device_name, error_message(err), ino, file, line);
	else
		fprintf(err_fp, "FUSE2FS (%s): %s at %s:%d.\n",
			device_name, error_message(err), file, line);
	fflush(err_fp);
